...
trendfunc <- function(event, time, eventname, strata = NULL, addstrataname = NULL, ybreaks = NULL) {
  r <- rsdata %>%
    filter(advhf == "Yes") %>%
    group_by(!!sym(strata), indexyear_fac) %>%
    summarise(
  ...
  if (strata == "all") {
    p <- r %>% ggplot(
      aes(x = indexyear_fac, y = rate, group = 1)
    ) +
  ...
  if (strata != "all") {
    p <- r %>% ggplot(
      aes(x = indexyear_fac, y = rate, group = level)
    ) +
