...
lvadspec <- paste0(outcommeta %>% filter(Variable == "sos_out_lvad") %>% pull(Code), "or FXL00 without FXM0 during the same hospitalization")
...
vars <- vars %>%
  ...
  mutate(
    ...
    Code = if_else(Variable == "Ventricular assist device implantation", lvadspec, Code),
...
footnotetext <- c(
  "Derived from diagnoses/operations in all positions, in-patient and out-patient speciality care within 5 years (3 years for malignant cancer) prior to index (for ICD-10) or 1987 (for ICD-9).",
  ...
  "Derived from diagnoses in all positions, in-patient and out-patient speciality care within 10 years prior to index using ICD-9/10 according to Ludvigsson, 2021 [9]."
)
